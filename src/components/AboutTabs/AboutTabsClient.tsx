...
export const AboutTabsClient = ({ aboutTabs }: { aboutTabs: AboutTab[] }) => {
	...
	return (
		...
		>
			<div className='flex flex-col justify-center items-start gap-2 xl:gap-4'>
				<h1 className='text-title font-bold'>{aboutTabs[activeTab].title}</h1>
				<p>{aboutTabs[activeTab].description}</p>
			</div>
			<div className='w-full min-h-[350px] py-16 bg-light-pink rounded-xl rounded-tl-[80px] border-[1px] border-black/10 hover:shadow-2xl transition-all duration-300 hover:scale-105 hidden lg:block relative overflow-hidden'>
				<Image
			...
			<div className='absolute bottom-[-40px] left-0 w-full py-6 bg-dark-gray sm:rounded-[50px] flex items-center justify-around'>
